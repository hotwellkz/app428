...
- **Телефония:** общие данные `hooks/useVoiceTelephonyData.ts` (Twilio + Telnyx + номера) для страниц `twilio` и `telnyx`.

Новая интеграция: добавить запись в `INTEGRATION_REGISTRY`, создать панель в `panels/`, зарегистрировать в `IntegrationDetailPage.tsx` (`PANELS`).
